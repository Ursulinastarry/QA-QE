 const users = [
  ...
  function dashboard(users) {
    const oneWeekAgo = new Date("2024-05-14T00:00:00"); 
    ...
    const unpopularPosts = activeUsers.map(user =>
        user.posts.filter(post => post.likes < 10)
    ).flat();  
    
    const totalLikes = unpopularPosts.reduce((sum, post) => sum + post.likes, 0);
